...
final public class WebEngine {
    ...
    private WebEngine(String url, boolean callLoad, String pin) {
        checkThread();
     ...
     */
    public void load(String url, String pin) {
        checkThread();
        ...
        if (url == null || url.equals("") || url.equals("about:blank")) {
            ...
            try {
                url = Util.adjustUrlForWebKit(url, pin);
    ...
    private final class LoadWorker implements Worker<Void> {
        ...
        @Override public final ReadOnlyObjectProperty<Worker.State> stateProperty() { checkThread(); return state.getReadOnlyProperty(); }
        private void updateState(Worker.State value) {
            checkThread();
            this.state.set(value);
            running.set(value == Worker.State.SCHEDULED || value == Worker.State.RUNNING);
        }
